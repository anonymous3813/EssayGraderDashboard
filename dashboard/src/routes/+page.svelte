<script lang="ts">
	...
	let selected = $derived<Run>(data.runs[0]);
	...
	let allEssays = $derived(
		selected.essays ? Object.values(selected.essays) : ((selected as any).results ?? [])
	);
	...
	let sysDiff = $derived(
		prev()
			? diffText(prev()!.metadata.system_instructions, selected.metadata.system_instructions)
			: null
	);
	...
	function toggleRow(i: number) {
		expandedRows = new SvelteSet(expandedRows);
...
<div class="shell">
	...
	<aside class="sidebar">
		...
		{#each data.runs as run, i (run.id)}
			<button class="run-btn" class:active={run === selected} onclick={() => selectRun(run)}>
				<div class="run-top">
					<span class="run-num">Run {data.runs.length - i}</span>
					<span
						class="mae-badge"
						class:good={run.metrics.mae <= 0.5}
						class:warn={run.metrics.mae > 0.5}
					>
						{fmt(run.metrics.mae)}
					</span>
				</div>
	...
	<main class="main">
		...
		<div class="header">
			<h1>{selected.id}</h1>
			<p class="subtitle">
				{fmtDate(selected.metadata.timestamp)} · {selected.metrics.n} essays · mean bias {selected
					.metrics.mean_bias >= 0
					? '+'
					: ''}{fmt(selected.metrics.mean_bias, 3)} pts
			</p>
		</div>
		...
		<div class="tabs">
			{#each [['metrics', 'Metrics & essays'], ['instructions', 'System instructions']] as [val, label] (val)}
				<button
					...
					onclick={() => (activeTab = val as typeof activeTab)}>{label}</button
				>
			{/each}
		</div>
		...
		{#if activeTab === 'metrics'}
			<!-- Summary cards -->
			<div class="cards">
				{#each [{ label: 'Essays', val: selected.metrics.n }, { label: 'Failed', val: selected.metrics.failed }, { label: 'MAE', val: fmt(selected.metrics.mae, 3) + ' pts' }, { label: 'RMSE', val: fmt(selected.metrics.rmse, 3) + ' pts' }, { label: 'Exact match', val: fmt(selected.metrics.exact_pct, 1) + '%' }, { label: 'Within ½ pt', val: fmt(selected.metrics.close_pct, 1) + '%' }, { label: 'Mean bias', val: (selected.metrics.mean_bias >= 0 ? '+' : '') + fmt(selected.metrics.mean_bias, 3) + ' pts' }, { label: 'Problematic', val: allEssays.filter((e) => Math.abs(e.total_diff) >= 1).length }] as card (card.label)}
					<div class="card">
				...
				<div class="filter-group">
					{#each [['all', 'All'], ['exact', 'Exact only'], ['problematic', 'Problematic']] as [val, label] (val)}
						<button
							...
							onclick={() => (filter = val as typeof filter)}>{label}</button
						>
					{/each}
				</div>
			</div>
				...
				<table>
					...
					<tbody>
						{#each filteredEssays as essay, i (i)}
							...
							<!-- Main row -->
							<tr class:flagged={absDiff >= 1} class="essay-row" onclick={() => toggleRow(i)}>
								...
								{#each criteriaKeys as k (k)}
									...
									<td
										class="num"
										class:pos={cd?.diff != null && cd.diff > 0}
										class:neg={cd?.diff != null && cd.diff < 0}
									>
													...
													<div class="criterion-header">
														...
														<div class="criterion-scores">
															<span class="score-label"
																>Real <strong>{cd.real}/{cd.possible}</strong></span
															>
															<span class="score-label"
																>AI <strong>{cd.ai ?? '—'}/{cd.possible}</strong></span
															>
															<span
																class="diff-pill"
